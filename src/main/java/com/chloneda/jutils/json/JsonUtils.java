...
public class JsonUtils {
    ...
    static {
        try {
            ...
            InputStream in = JsonUtils.class.getClassLoader().getResourceAsStream("application.properties");
            if (in != null) {
                properties.load(in);
                jsonClassType = properties.getProperty(JSON_CLASS_TYPE);
            } else {
                log.warning("未找到application.properties");
                in = JsonUtils.class.getClassLoader().getResourceAsStream("application.yml");
                if (in != null) {
                    Yaml yaml = new Yaml();
