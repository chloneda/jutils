package com.chloneda.utils.filesystem.ftp;
...
public class FTPConstant {
    ...
    static {
        REPLY_CODE.put(120, "服务已就绪，在 nnn 分钟后开始。");
        ...
        REPLY_CODE.put(221, "服务关闭控制连接。如果适当，请注销。");
        REPLY_CODE.put(225, "数据连接打开，没有进行中的传输。");
        REPLY_CODE.put(226, "关闭数据连接。请求的文件操作已成功（例如，传输文件或放弃文件）。");
        REPLY_CODE.put(227, "进入被动模式 (h1,h2,h3,h4,p1,p2)。");
        ...
        REPLY_CODE.put(426, "Connection closed; transfer aborted.");
        REPLY_CODE.put(450, "未执行请求的文件操作。文件不可用（例如，文件繁忙）。");
        REPLY_CODE.put(451, "请求的操作异常终止：正在处理本地错误。");
        REPLY_CODE.put(452, "未执行请求的操作。系统存储空间不够。");
        ...
        REPLY_CODE.put(550, "未执行请求的操作。文件不可用（例如，未找到文件，没有访问权限）。");
        REPLY_CODE.put(551, "请求的操作异常终止：未知的页面类型。");
        REPLY_CODE.put(552, "请求的文件操作异常终止：超出存储分配（对于当前目录或数据集）。");
        REPLY_CODE.put(553, "未执行请求的操作。不允许的文件名。");
    }
}
